...
export function FireView({ data, onChange }: { data: AppData; onChange: (data: AppData) => void }) {
  ...
  function updateFireNumber(key: keyof AppData['fire'], value: string) {
    ...
    onChange({ ...data, fire: { ...data.fire, [key]: key === 'withdrawalRate' ? number / 100 : number } });
  }
    ...
    <section className="fire-view">
      <div className="dashboard-hero fire-hero">
        ...
        <div className="hero-delta">
          ...
          <small>{analysis.estimatedMonthsToFire === null ? '暂无法估算达成时间' : `预计还需 ${analysis.estimatedMonthsToFire} 个月`}</small>
        </div>
      </div>
      ...
      <div className="fire-grid">
        ...
        <section className="chart-card">
          ...
          <div className="contribution-list">
            ...
            <div><span>月均增长</span><strong>{formatMoney(analysis.monthlyGrowth)}</strong></div>
